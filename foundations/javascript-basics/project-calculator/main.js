    ...
    const calculator = {
        ...
        init() {
            ...
            document.querySelectorAll('#calc-operators-basic .operator-basic')
                .forEach(operatorNode => {
                    operatorNode.addEventListener('click', this.handleOperatorClick.bind(this), false);
                });
            // Operands
            document.querySelectorAll('#calc-operands .operand')
                .forEach(operandNode => {
                operandNode.addEventListener('click', this.handleOperandClick.bind(this), false);
            });
            // Other Buttons
            document.getElementById('clear-btn')
                .addEventListener('click', this.handleClearClick.bind(this), false);
            // Keyboard
        ...
        handleKeyUp(e) {
            ...
            if (!isNaN(keyName) || keyName === '.') {
                // Click corresponding button on calculator to trigger event and show css effects
                element = document.querySelector(`.operand[data-operand="${keyName}"]`);
            } else { // Else key is NOT a number
                // Check for operators
                // Key: keyboard key name
                // Value: data attribute for corresponding calculator operator button element
                ...
                const dataAttribute = operatorConversion[keyName];
                if (dataAttribute !== undefined) {
                    element = document.querySelector(`.operator-basic[data-operator="${dataAttribute}"]`)
                }
            }
            if (element) {
                element.click();
        ...
        divide(num1, num2) {
            ...
            if (num2 === 0){
                ...
                // Reset currOperand to let user input another number to divide by instead of zero
                this._currOperand = null;
                return;
            }
        ...
        setDisplayText(newText) {
            // Adjust newText if it's a number
            if (!isNaN(newText)) {
                // Convert to actual Number if newText is String representation of Number
                if (typeof(newText) === 'string') {
                    newText = +newText;
                }
                // Limit number of digits
                newText = newText.toPrecision(12);
                // If number has a decimal, remove any trailing zeros
                if (newText.includes('.')) {
